...
const EstOpinionForm = ({
  ...
  return (
    <div>
      <div className="my-12">
        <Divider orientation="left">نماینده نفت</Divider>
        <Form
          form={form}
          onFinish={onFinish}
          disabled={visit_Type !== 1}
          layout="vertical"
          initialValues={{ samt_test_item: [], est_test_item: [] }}
        >
          <Form.Item label="نظر نهایی" name="naft_Opinion_ID">
            <CommentWorkflowSelectField
              onChange={(value) => setDisplayTestItem(value)}
            />
          </Form.Item>
          {displayTestItem == 3 && (
            <Form.Item label="فاکتور های آزمون" name={"naft_test_item"}>
          ...
          {request.naft_result_modify_date_time != "" && (
            <Form.Item label="آخرین ویرایش" name="naft_result_modify_date_time">
