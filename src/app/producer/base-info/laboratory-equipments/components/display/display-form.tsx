...
export default function DisplayForm({
  ...
  useEffect(() => {
    if (data) {
      const newData = { ...data };
      newData.lab_HasAtmosphericDistillation = renderValue(
        data.lab_HasAtmosphericDistillation
      );
      newData.lab_HasVacuumDistillation = renderValue(
        data.lab_HasVacuumDistillation
      );
    ...
    <Spin spinning={isLoading}>
      <Form disabled layout="vertical" form={form}>
        <Row gutter={[16, 16]}>
          ...
          <Col xs={24} md={8}>
            <Form.Item name="lab_HasVacuumDistillation" label="تقطیر در خلاء">
              <Input size="large" placeholder="وارد کنید" />
            </Form.Item>
          </Col>
          <Col xs={24} md={8}>
            <Form.Item name="lab_HasPourPoint" label="نقطه ریزش">
              <Input
                size="large"
        ...
        <Row gutter={[16, 16]}>
          <Col xs={24} md={8}>
            <Form.Item name="lab_HasFlashPoint" label="نقطه اشتعال">
              <Input size="large" placeholder="وارد کنید" />
            </Form.Item>
          </Col>
          <Col xs={24} md={8}>
            <Form.Item name="lab_HasViscometer" label="ویسکومتر">
              <Input size="large" placeholder="وارد کنید" />
            </Form.Item>
          </Col>
          <Col xs={24} md={8}>
        ...
        <Row gutter={[16, 16]}>
          ...
          <Col xs={24} md={8}>
            <Form.Item name="lab_HasMeasureColor" label="اندازه گیری رنگ">
              <Input size="large" placeholder="وارد کنید" />
            </Form.Item>
          </Col>
          <Col xs={24} md={8}>
