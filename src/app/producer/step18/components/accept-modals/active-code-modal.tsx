...
function ActiveCodeModal({
  ...
  const openBoxReuest = UseBoxOpen();

  const handleFormSubmitCode = async (values: any) => {
    values.uid = recordUid;

    const res = await openBoxReuest.trigger();
    ...
    if (res) {
      ...
      form.resetFields();
      router.push(
        `/laboratory-panel/request-list/test-result-lab/${recordUid}`
      );
    }
  };
  ...
  return (
        ...
        <div>
          ...
          {openBox ? (
            <div className="font-normal text-sm">
              جهت باز کردن درب جعبه، لطفا کد پبامک شده را وارد نمایید
            </div>
          ) : (
            <div className="font-normal text-sm">
              جهت دریافت رمز، لطفا کد بارکد را وارد نمایید.
            </div>
          )}
        </div>
      }
    ...
    >
      {openBox ? (
        <SmsCodeForm form={form} onFinish={handleFormSubmitCode} />
      ) : (
        <ReceivePasswordForm />
      )}
    </Modal>
