...
const Index = ({ data }: PropsType) => {
  const validateData =
    formMakerApi.ProducerFormsGetDocSchemaByUid.type1Res.safeParse(data);
  ...
  const renderCard = () => {
    ...
    cardData.map((item, cardIndex) => {
      view.push(
        <Typography className="mb-5 font-semibold text-lg text-right">
          {item.Title}
        </Typography>
      );
      ...
      item.ListTable?.map((table) => {
        const columns: TableColumnsType<any> | undefined = table?.Header?.map(
          (column) => ({
        ...
        view.push(
          <Card className="bg-gray-100">
            ...
            <div className="w-full">
              <Table columns={columns} dataSource={table?.Values} />
            </div>
          </Card>
        );
      });
      ...
      for (let key in item.Table?.Values) {
        ...
        if (column?.Value)
          desc.push(
            <Descriptions.Item span={6} label={column?.Value}>
              {value}
            </Descriptions.Item>
          );
      }
      ...
      if (desc.length > 0)
        view.push(
          <Card className="bg-gray-100">
            <Descriptions className="text-right text-secondary-500" column={6}>
              {desc}
            </Descriptions>
          </Card>
        );
      ...
      item.Media?.Images?.map((image, index) =>
        imageView.push(
          <Col span={4}>
            <Image
              key={index}
      ...
      item.Media?.Videos?.map((video, index) =>
        videoView.push(
          <Col span={4}>
            <Image
              alt="Base64 move"
...
const RenderImages = ({ imagesUid }: { imagesUid: string[] }) => {
  ...
  return imageQueries.map((image, index) => (
    <Image
      key={index}
      loading="lazy"
